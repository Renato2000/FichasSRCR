...
% exemplo de conhecimente interdito
servico(np9, zulmira).
excecao(servico(S, E)) :- servico(np9, E). % define o serviço como uma exceção
nulo(np9). % define o tocken como nulo
+servico(S, E) :: (
    solucoes(S, (servico(S, zulmira), nao(nulo(S))), L),
    comprimento(L, N),
    N == 0
    ).
...
% excecao(ato(sotura, E, josefa, D)) :- pertence(E, [maria, mariana]), pertence(D, [terca, sexta]).
...
+ato(A, E, U, D) :: (
    D \= feriado,
    solucoes((A,E,U,D), (ato(A,E,U,D)), S),
    comprimento(S, N),
    N == 1
    ).

-servico(_, Nome) :: (
    solucoes(Nome, (ato(_,Nome,_,_)), S),
    comprimento(S, N),
    N == 0
    ).
